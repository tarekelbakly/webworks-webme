...
if(isset($_REQUEST['action']) && $_REQUEST['action']){
	...
	foreach($fields as $name=>$field){
		...
		if($field->required && (!isset($_REQUEST[$name]) || !$_REQUEST[$name]))$errors[]='You must enter the "'.htmlspecialchars($name).'" field.';
	}
	// }
	...
	if(count($errors)){
		...
		// }
		// { generate invoice
		include_once SCRIPTBASE . 'ww.incs/Smarty-2.6.26/libs/Smarty.class.php';
		$smarty = new Smarty;
		...
		$c.=online_store_generate_paypal_button($PAGEDATA,$id,$total);
		// }
