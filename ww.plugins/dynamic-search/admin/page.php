...
if(@$_POST['dynamic_submit']){
	...
	if(in_array($add,$catags)) $error='That category already exists.';
        ...
        elseif(!$error){
                if($SS['cat']!='') $SS['cat']=$SS['cat'].','.$add;
                else $SS['cat']=$add;
                dbQuery('update site_vars set value="'.$SS['cat'].'" where name="cat"');
                $error='Category Added';
        }
}

$delete=@$_GET['dynamic_delete_cat'];
if($delete!=''){
	$num=0;
	foreach($catags as $catag){
		if($catag!=$delete){
			$num++;
			if($num==1) $SS['cat']=$catag;
			else $SS['cat'].=','.$catag;
		}
	}
	dbQuery('update site_vars set value="'.$SS['cat'].'" where name="cat"');
