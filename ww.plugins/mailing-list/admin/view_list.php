...
function get_contents($type,$status,$name){
	global $FIELD;
	$f='';
	if($type=='headers'){
		if($FIELD['col_name']==1) $f.='<th>Name</th>';
		if($FIELD['dis_pend']==1) $f.='<th>Status</th>';
		return $f;
	}
	elseif($type=='columns'){
		if($FIELD['col_name']==1) $f.='<td id="name">'.htmlspecialchars($name).'</td>';
		if($FIELD['dis_pend']==1) $f.='<td id="status">'.htmlspecialchars($status).'</td>';
		return $f;
	}
}
...
foreach($lists as $list){
	$num++;
	echo '<tr><td id="num">'.$num.'</td><td id="email">'.htmlspecialchars($list['email']).'</td>'.get_contents('columns',$list['status'],$list['name']).'<td id="delete"><a href="'.$_url.'&mailing_list=delete&id=!'.htmlspecialchars($list['id']).'">[x]</a></td></tr>';
}
