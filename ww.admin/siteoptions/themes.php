...
// { handle actions
if($action=='set_theme'){
	if(is_dir(BASEDIR . 'ww.skins/' . $_REQUEST['theme'])){
...
// }
// { samples
	$dir=new DirectoryIterator(BASEDIR . 'ww.skins');
	$themes_found=0;
	foreach($dir as $file){
		...
		if(file_exists(BASEDIR . 'ww.skins/' . $file . '/screenshot.png'))echo '<img src="/ww.skins/'.htmlspecialchars($file).'/screenshot.png" />';
