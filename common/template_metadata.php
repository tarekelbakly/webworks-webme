...
function template_get_metadata($template,$PAGEDATA){
	...
	if(strpos($template,'class="tabs"')!==false)$plugins_to_load[]='"tabs":1';
	if(strpos($template,'class="showhide"')!==false)$plugins_to_load[]='"showhide":1';
	if(strpos($template,'class="fontsize_controls"')!==false)$plugins_to_load[]='"fontsize_controls":1';
	if(strpos($template,'class="imagefader"')!==false)$plugins_to_load[]='"imagefader":1';
	if(strpos($template,'class="inputdate"')!==false)$plugins_to_load[]='"inputdate":1';
	if(strpos($template,'class="removeRowIfEmpty"')!==false)$plugins_to_load[]='"removeRowIfEmpty":1';
	if(strpos($template,'class="sc_ssearch"')!==false)$plugins_to_load[]='"sc_ssearch":1';
